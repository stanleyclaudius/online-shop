...
<div class="container mx-auto mt-16 mb-24">
	...
	<p class="text-md text-gray-500 text-center">Reset password for <b>{{ $email }}</b> account.</p>
	<div class="flex items-center justify-center mt-12">
		<form action="/reset/password/{{ $email }}/{{ $token }}" method="post" style="box-shadow: 1px 1px 15px rgba(0,0,0,.3);" class="px-6 py-6 rounded">
			@csrf
			...
			<button type="submit" class="mt-5 rounded bg-black text-white px-2 py-2 transition duration-150 ease-in-out hover:bg-gray-800">Reset</button>
		</form>
	</div>
</div>
@endsection