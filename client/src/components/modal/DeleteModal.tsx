...
const DeleteModal: React.FC<IProps> = ({ deleteModalRef, openDeleteModal, setOpenDeleteModal }) => {
  return (
    <div className={`${openDeleteModal ? 'opacity-100' : 'opacity-0'} ${openDeleteModal ? 'pointer-events-auto' : 'pointer-events-none'} transition-opacity fixed top-0 left-0 bottom-0 right-0 bg-[rgba(0,0,0,.7)] z-[9999] flex justify-center items-center px-5 font-opensans`}>
      <div ref={deleteModalRef} className={`${openDeleteModal ? 'translate-y-0' : '-translate-y-12'} transition-transform w-full max-w-[400px] bg-white rounded-md flex items-center flex-col p-7 gap-5`}>
        ...
        <div className='flex items-center gap-7'>
          ...
          <button className='rounded-md px-5 py-2 rounded-md bg-gray-100 hover:bg-gray-200 transition-[background] text-sm' onClick={() => setOpenDeleteModal(false)}>
            Cancel
          </button>
        </div>
      </div>
